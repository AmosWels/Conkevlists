...
$organ_citation_Taken = TPcitationProfilefieldsMappings::model()->findAll("status IN ('T','C','L') and data_entrant='$userid' and profilefield = '13'"); // getting takenup, corrected and discarded citations

$person_citation_test = TPcitationProfilefieldsMappings::model()->findAll("status = 'A' and profilefield = '13'"); // getting new citations

$rejectedemployment = TPersonEmploymentDetails::model()->findAll("maker = '$userid' and status = 'R'"); // getting rejected position attributes
?>
                        ...
                        <div class="row search-tabs-row search-tabs-container grey lighten-4">
                            <div class="col s12 m3 16">                                
                                <ul class="tabs">
                                    ...
                                    <li class="tab col s4"><a href="#rejected">Rejected<span class="red white-text circle">&nbsp <?php echo count($rejectedemployment); ?> &nbsp;</span></a></li>
                                </ul>
                            </div>
                        </div>
                    </div>    
...
//                                            nationality name
                                            ...
                                            ?>
                                            <?php if($Tstatus=='C' ){?>
                                        <tr class="green-text" onclick="location.href = '<?php echo @Yii::app()->baseUrl; ?>/index.php?r=dataEntry/Entrant/employment/search&id=<?php echo $code->encode($Trecord->id); ?>'">
                                        <?php }elseif($Tstatus=='L'){ ?>
                                         <tr class="red-text" onclick="location.href = '<?php echo @Yii::app()->baseUrl; ?>/index.php?r=dataEntry/Entrant/employment/search&id=<?php echo $code->encode($Trecord->id); ?>'">   
                                        <?php } else{ ?>
                                         <tr onclick="location.href = '<?php echo @Yii::app()->baseUrl; ?>/index.php?r=dataEntry/Entrant/employment/search&id=<?php echo $code->encode($Trecord->id); ?>'">
                                        <?php } ?>
                                ...
                                <table id="example2" class="display responsive-table datatable-example">
                                    ...
                                    <tbody>
                                        ...
                                        foreach ($person_citation_test as $record) {
                                            ...
                                            $TpersonValue = Tperson::model()->findByPk($person);
                                            $personName = $TpersonValue->name;
//                                            nationality name
                                            $nid = $TpersonValue->nationality;
                    ...
                    <div class="card z-depth-0">
                            <div class="card-content">
                                <table id="example3" class="display responsive-table datatable-example">
                                    ...
                                    <tbody>
                                        <?php
                                        if (!empty($rejectedemployment)) {    
                                            $levelname = '';
                                        foreach ($rejectedemployment as $record) {
//                                            citation details
                                            $citationid = $record->citation;
                                            $citationValue = TPersonCitation::model()->findByPk($citationid);
                                            $citationtitle = $citationValue->title;
                                            
                                            $posn = $record->person_position;
                                            $posnValue = TPersonpositions::model()->findByPk($posn);
                                            $posnName = $posnValue->name;
...
//                                            nationality name
                                            ...
                                            $nationalityname = $nationalityvalue->native;
                                            ?>  
                                        <tr class="red-text" onclick="location.href = '<?php echo @Yii::app()->baseUrl; ?>/index.php?r=dataEntry/Entrant/employment/correctrejectedposition&id=<?php echo $code->encode($record->id); ?>'">
                                                <td><?php echo $posnName; ?></td>
                                                <td><?php echo $personName; ?></td>
                                                <td><?php echo $nationalityname; ?></td>
