...
$citation = TPersonCitation::model()->findByPk("$citeid");
//$citation = TPersonCitation::model()->findByPk("$citeid");

$personid = $citation->person;
$personValue = Tperson::model()->findByPK($personid);
$personname = $personValue->name;
//getting people attributes
$researches = TPeopleprofilefields::model()->findAll("status='A'");
                            ...
                            <div class="col s12 m12 l10 left">
                                <h5 class="" style="font-size: 16px">                                    
                                    <div class="breadcrumbs">
                                        ...
                                        <span><?php echo $citation->title; ?></span>
                                    </div>
                                </h5>
                            </div>
                        </div>
                            ...
                            <div class="col s10 m10">                                
                                <ul class="tabs">
                                    ...
                                    <?php if($citation->status== 'R'){ ?>
                                        ...
                                        <small class="grey-text small" style="margin-left: 100px;"> Rejection Reason &rtrif; <span style="font-size: 14px; color: red;"><?php if(strlen($citation->rejectedreason_dataEntry)>70){ $reason = substr($citation->rejectedreason_dataEntry, 0, 70) . '...'; 
                                        ...
                                        <?php } else{ echo $citation->rejectedreason_dataEntry;} ?> </span></small>
                                    <?php } else {?>
                                            ...
                                            if (!empty($mappings)) {
                                                ...
                                                <a class="black-text">
                                                    ...
                                                    foreach ($mappings as $item) {  
                                                        $mapping_set .= $item->profilefield . ',';
                                                        $mapping_setid = $item->profilefield;
                                                        $mappingValue = TPeopleprofilefields::model()->findByPk($mapping_setid);
                                                        $set_names .=  $mappingValue->name. ', ';
                                                        $newset = $mappingValue->name;
//                                                      echo rtrim($set_names, ', ');
//                                                      
//                                                        checking if attribute is active or not
                                                        $attr_status = TPeopleprofilefields::model()->findByAttributes(array('id'=>$mapping_setid,'status'=>'C'));
                                                        
                                                        if(empty($attr_status)){  ?>
                                                      <li><?php echo $newset; ?></li>
                                                        <?php } else{ ?>
