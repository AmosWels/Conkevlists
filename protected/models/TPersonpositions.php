...
class TPersonpositions extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name, organization, level, citation, start_date, maker', 'required'),
			array('level, citation', 'numerical', 'integerOnly'=>true),
			array('name', 'length', 'max'=>300),
			array('organization, maker', 'length', 'max'=>25),
			array('supervisor', 'length', 'max'=>15),
			array('status', 'length', 'max'=>1),
			array('supervisor_reject_reason, dataEntrant_discard_reason', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, organization, level, citation, start_date, end_date, maker, supervisor, status, supervisor_reject_reason, dataEntrant_discard_reason', 'safe', 'on'=>'search'),
		);
	}
